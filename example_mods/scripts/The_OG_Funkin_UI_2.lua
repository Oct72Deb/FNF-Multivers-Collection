...
--Short animation for non bf characters in bf's place

local ShortNonBfFrames = false --(you can basically disable it if it's annoying for you)
local isTain = true
local holdTimer = 0 --maybe useless idk

function onUpdatePost(e)
  if ShortNonBfFrames and not (getProperty('boyfriend.curCharacter'):find('bf') or getProperty('boyfriend.curCharacter'):find('boyfriend')) then
	if isTain and getProperty('boyfriend.holdTimer') ~= 0 and string.find(getProperty('boyfriend.animation.curAnim.name'), 'sing') then
		setProperty('boyfriend.holdTimer', getProperty('boyfriend.holdTimer') + (e * 1.43))

  elseif ShortNonBfFrames and (getProperty('boyfriend.curCharacter'):find('spooky_kids') or getProperty('boyfriend.curCharacter'):find('spooky')) then
	if isTain and getProperty('boyfriend.holdTimer') ~= 0 and string.find(getProperty('boyfriend.animation.curAnim.name'), 'sing') then
		setProperty('boyfriend.holdTimer', getProperty('boyfriend.holdTimer') + (e * 1.5))
	end
	end
end
end
